...
static void cvt_info(FILE_INFO *info, char **buf)
{
  ...
  if(humanReadable == 0 || info->stat.st_size < 1024)
  {
    fprintf(stream, "%.10s %3ld %-8.8s %8ld %.12s %s",
  ...
  else if (humanReadable && info->stat.st_size > 1024 && info->stat.st_size < 1048576)
  {
    fprintf(stream, "%.10s %3ld %-8.8s %8ldK %.12s %s",
	  cvt_mode(info->stat.st_mode),
	  info->stat.st_nlink,
	  pw != NULL ? pw->pw_name : "",
	  info->stat.st_size / 1000,
	  ctime(&info->stat.st_mtime)+4,
	  n);
  } 
  else
  {
    fprintf(stream, "%.10s %3ld %-8.8s %8ldM %.12s %s",
	  cvt_mode(info->stat.st_mode),
	  info->stat.st_nlink,
	  pw != NULL ? pw->pw_name : "",
	  info->stat.st_size / 1000000,
	  ctime(&info->stat.st_mtime)+4,
	  n);
  }
  fclose(stream);
